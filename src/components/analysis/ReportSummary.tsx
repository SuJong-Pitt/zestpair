...
export default function ReportSummary({ result, className }: ReportSummaryProps) {
    ...
    const handleShare = useCallback(async () => {
        ...
        insights.forEach((insight, idx) => {
            ...
            const title = isKo ? meta.labelKo : meta.labelEn;
            text += `📍 ${title}\n${insight}\n\n`;
        });

        // 2. 추천 대상
        if (result.recommendation_targets?.length) {
            text += `🎯 ${isKo ? '추천 복용 대상' : 'Recommended For'}\n`;
            result.recommendation_targets.forEach(t => text += `- ${t}\n`);
            text += `\n`;
        }

        // 3. 생활 습관
        if (result.lifestyle_guidelines?.length) {
            text += `💡 ${isKo ? '생활 습관 시너지' : 'Lifestyle Synergy'}\n`;
            result.lifestyle_guidelines.forEach(g => text += `- ${g}\n`);
            text += `\n`;
        }

        // 4. 타임라인
        if (result.expected_timeline) {
            text += `⏳ ${isKo ? '4주 기대 효과' : '4-Week Journey'}\n`;
            text += `Week 1: ${result.expected_timeline.week1}\n`;
            text += `Week 2: ${result.expected_timeline.week2}\n`;
            text += `Week 4: ${result.expected_timeline.week4}\n\n`;
        }

        text += `${isKo ? '분석 결과 더 보기' : 'View Full Analysis'}: ${window.location.href}`;

        if (navigator.share) {
            try {
                await navigator.share({
                    title: isKo ? 'ZestPair 분석 결과' : 'ZestPair Analysis',
                    text: text
                });
            } catch (err) {
            ...
            alert(isKo ? "리포트 전문이 클립보드에 복사되었습니다! 카톡 등에 붙여넣어 공유하세요." : "Full report copied to clipboard!");
        }
    }, [isKo, score, insights, hasWarning, result, isHighEnd]);
    ...
    return (
        <div className={cn("relative", className)}>
            ...
            <div className="flex items-center gap-3 mb-5 px-1">
                ...
                {/* Single Premium Share Button ✨ */}
                <div className="ml-auto">
                    <button
                        onClick={handleShare}
                        className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/20 hover:text-emerald-300 transition-all group"
                    >
                        <Share2 size={12} className="group-hover:rotate-12 transition-transform" />
                        <span className="text-[10px] font-black tracking-tight uppercase">{isKo ? '공유하기' : 'SHARE'}</span>
                    </button>
                </div>
